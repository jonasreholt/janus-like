...
added that simply calls and uncalls the \lsin{fib} procedure.
...
is compiled using the flag \lsin{-O0} to avoid it doing any optimizations. This lack of extra
...
\begin{table}[H]
    ...
    \begin{tabular}{|l||r|r|r||r|r|r||r|r|r|}
        \hline
        \multirow{2}{*}{Program}& \multicolumn{2}{c|}{Compile Time (ms)} & \multirow{2}{*}{Diff (X)} & \multicolumn{2}{c|}{Runtime (ms)} & \multirow{2}{*}{Diff (\%)} & \multicolumn{2}{c|}{Assembly Lines} & \multirow{2}{*}{Diff (\%)} \\
...
remove respectively three (all but the loop checks), one (missing a \lsin{dealloc} and loop checks),
